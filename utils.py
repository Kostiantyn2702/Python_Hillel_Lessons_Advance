...
def lucky_number(ticket: str) -> bool:
    ...
    return True if sum_one == sum_second else False
...
if __name__ == '__main__':
    ...
    assert volume_of_sphere(10) == "Wrong data type"
    assert volume_of_sphere(5.5) == 696.91
    assert volume_of_sphere(20.0) == 33510.32


# Не совсем понимаю зачем эта функция.
# Можно сразу закинуть в параметры days_diff(datetime.date(2020, 1, 5), datetime.date(2020, 1, 1)
# И код вышел бы в 2 строчки
# Но удобнее вводить так, но может кому-то удобнее по-другому.
...
def prs(client_choice: str) -> bool:
    ...
    if client_choice != "paper" and client_choice != "rock" and client_choice != "scissors":
...
if __name__ == '__main__':
    assert prs(12) == "Wrong data type"
    assert prs("papers") == "Wrong atribute"
